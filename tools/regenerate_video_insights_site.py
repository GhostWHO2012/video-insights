...
<main>
  <section class="hero"><div class="wrap">
    <div class="hero-head">
      ...
      <div class="mascot-stage" aria-hidden="true"><div class="mascot-sprite" id="mascotSprite"></div></div>
    </div>
...
const mascot=document.getElementById('mascotSprite');const mascotStage=document.querySelector('.mascot-stage');
if(mascot&&mascotStage){{const cols=16,rows=16,cellW=256,cellH=256;function clamp(v,min,max){{return Math.max(min,Math.min(max,v));}}function face(x,y){{const r=mascotStage.getBoundingClientRect();const cx=r.left+r.width/2;const cy=r.top+r.height/2;const nx=clamp((x-cx)/(r.width/2),-1,1);const ny=clamp((y-cy)/(r.height/2),-1,1);const col=Math.round((nx+1)/2*(cols-1));const row=Math.round((ny+1)/2*(rows-1));mascot.style.backgroundPosition='-'+(col*cellW)+'px -'+(row*cellH)+'px';}}window.addEventListener('pointermove',e=>face(e.clientX,e.clientY),{{passive:true}});face(window.innerWidth*.72,160);}}
</script>
"""
